...
class local( trajectory ):
    ...
    def add_rdf(self, cutoff, bins):
        distances = np.sqrt( (self._distance_matrix**2).sum(axis = self._distance_matrix.get_axis_num('comp')) )
        ...
        self._print('\n')
        self._rdf = xr.DataArray(rdf_traj, coords = [self.timesteps, self.ids, cutoff_array[0:-1]], dims = ['ts', 'id', 'distance'])
